...
public class HttpResponse<T> {
    ...
    private HttpResponse(HttpResponseBuilder<T> builder) {
        ...
        if (headers != null) {
            ...
            try {
                ...
                throw new TransportException("Invalid Location header TransportExceptionvalue in response; cannot complete the request (location="
                        + locationValue + ')', e);
            }
        } else {
            this.location = null;
    ...
    public static HttpResponseBuilder<Void> anHttpResponse(int statusCode) {
        return new HttpResponseBuilder<>(statusCode);
    }

    public static <T> HttpResponseBuilder<T> anHttpResponse(int statusCode, Class<T> entityType) {
        return new HttpResponseBuilder<T>(statusCode);
    }

    public static <T> HttpResponseBuilder<T> anHttpResponse(int statusCode, T entity) {
